...
export function CameraInitLoader({ isLoading }: Props) {
  ...
  useEffect(() => {
    ...
    const interval  = setInterval(() => {
      ...
      if (p >= 100) { clearInterval(interval); setFadeOut(true) }
    }, 16)
    ...
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: 300, padding: '0 20px' }}>
        <div style={{ width: '100%', height: 4, background: 'var(--border)', borderRadius: 2, marginBottom: 24, overflow: 'hidden' }}>
          <div style={{ height: '100%', background: 'var(--primary)', borderRadius: 2, width: `${progress}%`, transition: 'width 0.1s linear' }} />
        </div>
        <div style={{ textAlign: 'center', fontSize: 14, color: 'var(--grey)', fontWeight: 500 }}>{status}</div>
        <div style={{ textAlign: 'center', fontSize: 12, color: 'var(--primary)', marginTop: 12, fontWeight: 600, opacity: 0.7 }}>
          {Math.round(progress)}%
        </div>
      </div>
    </div>
